import { DEFAULT_LANGUAGE } from './constants.js?v=639255192815845614';
...
// How much of the full transcript this client has already received, in characters - sent back to
// LiteGetStream as transcriptCursor so the server returns only the new text since this point
// instead of the full transcript on every poll. Starts at 0 (a fresh page load, including a late
// joiner) so the very first response naturally contains everything said so far.
export let transcriptCursor = 0;
...
export function setTranscriptCursor(val) { transcriptCursor = val; }
