import { API_URL, mockObject, mockObject2, mockObject3, DEFAULT_LANGUAGE, languageData } from './constants.js?v=639254312593451259';
import {
    response,
    languageCode,
    setLanguageCode,
    ...
    transcriptCursor,
    setTranscriptCursor
} from './state.js?v=639254312593451259';
import {
    updateSidebarActiveStates,
    populateLanguageMenu,
    translate,
    buttonTapped
} from './ui.js?v=639254312593451259';
import { readLogic } from './speech.js?v=639254312593451259';
import { normalizeLang } from './utils.js?v=639254312593451259';
...
export function getTranscript() {
    $.support.cors = true;
    // transcriptCursor tells the server how much of the transcript this client already has, so it
    // returns only the new text since that point instead of the full transcript on every poll -
    // this is what keeps the response small over a multi-hour event. Omitting it (cursor 0, a fresh
    // page load) makes the server return everything said so far, which is also how a late joiner
    // naturally catches up with no separate history call needed.
    const separator = API_URL.includes('?') ? '&' : '?';
    const urlWithCursor = `${API_URL}${separator}transcriptCursor=${transcriptCursor}`;
    $.getJSON(urlWithCursor, function (data) {
        if (data) {
            if (data.transcript !== undefined) {
                ...
                // data.transcript is only the new delta since our last cursor - append it to what
                // we already have to reconstruct the full transcript, and correct response.input
                // (which updateResponseData just set to the raw delta) to the full value.
                const fullTranscript = transcript + (data.transcript || "");
                setTranscript(fullTranscript);
                if (typeof data.transcriptCursor === 'number') setTranscriptCursor(data.transcriptCursor);
                response.input = fullTranscript;

                // speechTranslation/N is a slower, sentence-stable, append-only feed meant for TTS -
                // unlike translation/N (a fast, frequently re-windowed feed meant for on-screen
                // reading), it only ever grows, which is what readLogic's word-diffing needs to
                // speak new words instead of restarting on every poll. Falls back to translation/N
                // for streams from an older broadcaster app that doesn't send the speech fields yet.
                let textToRead = "";
                if (languageCode === response.inputLanguage) textToRead = fullTranscript;
                else if (languageCode === response.outputLanguage) textToRead = data.speechTranslation || data.translation;
                else if (languageCode === response.outputLanguage2) textToRead = data.speechTranslation2 || data.translation2;
                else if (languageCode === response.outputLanguage3) textToRead = data.speechTranslation3 || data.translation3;
                else if (languageCode === response.outputLanguage4) textToRead = data.speechTranslation4 || data.translation4;
                else if (languageCode === response.outputLanguage5) textToRead = data.speechTranslation5 || data.translation5;
